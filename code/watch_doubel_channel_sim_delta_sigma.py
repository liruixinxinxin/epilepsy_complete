...
raw = mne.io.read_raw_edf(f'/home/ruixing/workspace/chbtar/chb/raw_selected/raw_selected/train/{name}',preload = True)
sampling_frequency = 250
interpfact = 250 #连续化时的采样率
refractory = 3e-4
dt = 0.01
tmin = int(time_dict[(name[0:8])][0])
tmax = int(time_dict[(name[0:8])][1])
raw = raw.filter(l_freq=0.1,h_freq=30)
raw = raw.resample(sfreq=250)
raw.pick(picks='all',exclude=['--0','--1','--2','--3','--4','--5'])
sampling_frequency=250
# crop = np.linspace(tmin-300,tmax+300,250*((tmax+300)-(tmin-300))+1)
crop = np.linspace(2985,3045,250*(60)+1)
# data = (raw.copy()).crop(tmin =tmin-300,tmax=tmax+300)
data = (raw.copy()).crop(tmin =2985,tmax=3045)
picks1= ['C3-P3']
picks2= ['C4-P4']
...
data_all_channel = data.get_data(units='uV')
data_average_channel = np.mean(data_all_channel,axis=0)

plt.subplots_adjust(left=None, bottom=None, right=None, top=None,
                wspace=0.2, hspace=0.5)
for num,elc in enumerate(picks_list):
    data_mul_channel =  data.get_data(units='uV',picks=elc)
    data_one_channel = np.mean(data_mul_channel,axis=0)
    x = crop
    y = data_one_channel
    label_x_play = np.linspace(tmin,tmin,2)
    label_x_end = np.linspace(tmax,tmax,2)
    label_y = np.linspace(-250,250,2)
    label_y_spike = np.linspace(0,1,2)
    axs[num].plot(x,y)
    axs[num].plot(label_x_play,label_y,color='red',linewidth=1)
    axs[num].plot(label_x_end,label_y,color='red',linewidth=1)
plt.show()
Fig.savefig('signal',dpi=1000)
    ...
    # else:
    #     # Get input signal 
    #     dt_original_data = 1/sampling_frequency#每采样一个点花的时间
    ...
    # x2 = np.linspace(tmin-300,tmax+300,int(((tmax+300)-(tmin-300))/dt))
#     if num == 0:
#         axs[num+2].plot(x2,synchronous_input[0],linewidth=0.1)
#         axs[num+2].plot(label_x_play,label_y_spike,color='red',linewidth=1)
#         axs[num+2].plot(label_x_end,label_y_spike,color='red',linewidth=1)
#         axs[num+3].plot(x2,synchronous_input[1],linewidth=0.1)
#         axs[num+3].plot(label_x_play,label_y_spike,color='red',linewidth=1)
#         axs[num+3].plot(label_x_end,label_y_spike,color='red',linewidth=1)
    
#     if num == 1:
#         axs[num+3].plot(x2,synchronous_input[0],linewidth=0.1)
#         axs[num+3].plot(label_x_play,label_y_spike,color='red',linewidth=1)
#         axs[num+3].plot(label_x_end,label_y_spike,color='red',linewidth=1)
#         axs[num+4].plot(x2,synchronous_input[1],linewidth=0.1)
#         axs[num+4].plot(label_x_play,label_y_spike,color='red',linewidth=1)
#         axs[num+4].plot(label_x_end,label_y_spike,color='red',linewidth=1)
