...
for i in tqdm(sorted(dir.rglob('*.edf'))):
    ...
    #14 samples per file
    if int((time_dict[i.parts[-1][0:-4]])[0]-70)<0:
        continue
    tmin = int((time_dict[i.parts[-1][0:-4]])[0]-70)
    tmax = int((time_dict[i.parts[-1][0:-4]])[0])
    #filter
    ...
    p = 1
    while(1):
        data_one_trail = data.copy().crop(tmin=tmin,tmax=tmin+5)#that means one sample's time is 5 seconds
        for j in picks_list:
        ...
        np.save(f'data/test_data/npy/neg/{i.parts[-1][0:-4]}-trail{p}_neg',one_trail_data)
        one_trail_data = []
        tmin = tmin + 5
        if(tmin > (tmax-5)):
            break
...
dt = 0.01
sample = []
neg_dir = Path('data/test_data/npy/neg')
...
for i in tqdm(sorted(neg_dir.rglob('*.npy')),colour='cyan'):
    array = np.load(str(i))
    ...
    np.save(f'data/test_data/spike/neg/{i.parts[-1][0:-4]}_spike',spike_array_one_trail)
    spike_array_one_trail = []