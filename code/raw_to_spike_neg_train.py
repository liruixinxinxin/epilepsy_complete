...
for i in tqdm(sorted(dir.rglob('*.edf'))):
    ...
    while(1):
        ...
        np.save(f'data/train_data/npy/neg/{i.parts[-1][0:-4]}-trail{p}_neg',one_trail_data)
        one_trail_data = []
        tmin = tmin + 5
        if(tmin > (tmax-5)):
            break
...
dt = 0.01
sample = []
neg_dir = Path('data/train_data/npy/neg')
...
for i in tqdm(sorted(neg_dir.rglob('*.npy')),colour='cyan'):
    ...
    np.save(f'data/train_data/spike/neg/{i.parts[-1][0:-4]}_spike',spike_array_one_trail)
    spike_array_one_trail = []